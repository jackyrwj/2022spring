...
if strcmp(textFile1,'none') == 0
    showTextItem = 1;
    textItems1 = importdata(textFile1);
else
    showTextItem = 0;
end
...
% Load the text file (optional)
if strcmp(textFile2,'none') == 0
    showTextItem = 1;
    textItems2 = importdata(textFile2);
else
    showTextItem = 0;
end
...
fprintf(outputfile, 'subID\t imageSet\t trial\t prompt\t target\t imageOrder\t response\t RT\n');

% Randomize the trial list
% randomizedTrials = randperm(nTrials);
randomizedTrials = (1:nTrials);
...
for t = randomizedTrials
    ...
    if showTextItem
        % Display text
        textString1 = textItems1{t};
        textDuration = 2; % How long to show text (in seconds)
        Screen('DrawText', window1, textString1, (W/2-200), (H/2), textColor);
...
%         Screen(tTextdisplay,'Close');
    ...
    if showTextItem
        % Display text
        textString2 = ['have ',textItems2{t},' ?'];
        textDuration = 1; % How long to show text (in seconds)
        Screen('DrawText', window1, textString2, (W/2-200), (H/2), textColor);
        tTextdisplay = Screen('Flip', window1);
...
%         Screen(tTextdisplay,'Close');
    ...
    fprintf(outputfile, '%s\t %s\t %d\t %s\t %s\t %s\t %s\t %f\n',...
        subID, imageFolder, t, textString1, textItems2{t}, orderStr, resp, rt);
